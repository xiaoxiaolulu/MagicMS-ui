    ...
    export default {
        ...
        watch: {
            value(val) {
                ...
                // this.errors.clear();
                this.formData = {
                    suiteName: this.$parent.rowData.suite_name,
        ...
        methods: {
            ...
            queryOutTable() {
                ...
                let outCaseData = null;

                getCaseList({params: {name: this.name}}).then((res) => {
                    outCaseData = res.data.data;

                    let casesId = [];
                    let caseList = this.$parent.rowData.case;
                    for(let i=0; i<caseList.length; i++){
                        casesId.push(caseList[i]["cases"]["id"]);
                    }

                    // 获取test_name集合
                    for(let i=0; i< outCaseData.length; i++){
                        for(let j=0; j< casesId.length; j++){
                            if (casesId[j]=== outCaseData[i]['id']){
                                this.inCaseTableData.push({"test_name": outCaseData[i]['test_name']})
                            }
                        }
                    }
                    ...
                    let testsList = [];
                    for(let i=0; i<outCaseData.length; i++){
                        testsList.push({"test_name":outCaseData[i]['test_name']});
                    }

                    // 删除已选中的case
                    for (let i = 0; i < testsList.length; i++) {
                        for (let j = 0; j < this.inCaseTableData.length; j++) {
                            if(JSON.stringify(testsList[i]) === JSON.stringify(this.inCaseTableData[j])){
                                testsList.splice(i, 1)
                            }
                        }
                    }

                    for(let i=0; i<testsList.length; i++){
                        this.outCaseTableData.push(testsList[i])
                    }
                    this.outTableLoading = false;
                    this.outTableData = this.outCaseTableData;
                    this.outTableTotal = this.outCaseTableData.length;
                }).catch((error) => {
                    this.$message({
                        showClose: true,
                        type: 'error',
